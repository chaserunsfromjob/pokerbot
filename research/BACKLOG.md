...
| 4 | Restricted rollout search improves multiway postflop choices | High / next coding priority: explicit river action values, then earlier streets | Known-response fixtures and paired returns/latency versus same policy without search | Strategy fusion, wrong opponent ranges, side-pot-specific values |
