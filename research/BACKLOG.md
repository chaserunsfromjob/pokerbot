...
| 3 | Named-player shrinkage improves decisions over the exact same unadapted policy | Named context distributions implemented; 7,680-hand prior/oracle/learned screen complete | Stationary probability prediction improves; learning's added chip return is inconclusive; stale/mistaken histories fail robustness stress | No forgetting, bet-size conditioning or card-dependent response model; implement the fixed recency pilot next |
