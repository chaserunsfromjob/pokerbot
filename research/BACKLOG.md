...
| 4 | Restricted rollout search improves multiway postflop choices | River and named-response screens complete; turn plan frozen, not implemented | Several passive-pool search gains remain exploratory; turn expands only modest measured coverage; test against river-only and original | Uniform holdings, wrong response models, fixed hero continuation, sampling noise; no held-out strength passage |
...
| Before search-policy confirmation | Extend confirmation to typed strategy configurations | Required integration; existing entry point reconstructs EquityConfig only | River/named and future turn policies round-trip; legacy runs resume; baseline and shared alpha ledger unchanged | Silently rebuilding the wrong policy or resetting the repeated-attempt error budget would invalidate promotion |
| Future learning study | Select or mix memory rates using public predictive losses | Original hypothesis prompted by the fixed-decay tradeoff; not implemented | New prespecified stationary/sparse/changing/mistaken tests, then separate chip-return comparisons | Model-selection noise and delayed detection; prediction gains need not improve poker decisions |

The next search intervention is specified in `TURN_SEARCH_PLAN.md`: isolate
turn-plus-river from river-only play, sample unseen future cards and hold learning
off. Current recency prediction evidence is in `RESPONSE_RECENCY_RESULTS.md`.
